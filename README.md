<h1 align="center">校园活动与机会平台</h1>

<p align="center">
  <b>让新生在三分钟内判断「这个活动值不值得去」</b>
</p>

<p align="center">
  <img alt="状态" src="https://img.shields.io/badge/%E7%8A%B6%E6%80%81-%E8%AE%BE%E8%AE%A1%E9%98%B6%E6%AE%B5-orange?style=for-the-badge">
  <img alt="代码" src="https://img.shields.io/badge/%E4%BB%A3%E7%A0%81-%E5%B0%9A%E6%9C%AA%E5%BC%80%E5%A7%8B-lightgrey?style=for-the-badge">
  <img alt="活动库" src="https://img.shields.io/badge/%E6%B4%BB%E5%8A%A8%E5%BA%93-26%20%E6%9D%A1-blue?style=for-the-badge">
  <img alt="协作约定" src="https://img.shields.io/badge/%E5%8D%8F%E4%BD%9C%E7%BA%A6%E5%AE%9A-AGENTS.md-blueviolet?style=for-the-badge">
  <img alt="许可证" src="https://img.shields.io/badge/%E8%AE%B8%E5%8F%AF%E8%AF%81-%E5%BE%85%E5%AE%9A-lightgrey?style=for-the-badge">
</p>

## 概述

面向在校学生、重点考虑新生使用体验的**校园活动与机会平台**。

产品要解决三件事：让同学能**发现**合适的活动与机会、能**理解**它是否值得参与、能低成本地**发布**活动并招募伙伴；同时处理多来源信息进入平台后带来的真实性、重复、过期与营销噪声问题。

## 项目状态

**当前处于设计阶段，尚未开始编码。** 仓库内容为需求、产品设计方案与结构化活动数据。

- 设计方案已迭代至 **v3.0**，见 [design/产品设计方案.md](design/产品设计方案.md)
- 活动数据已结构化为 26 条种子数据，见 [design/activities.seed.json](design/activities.seed.json)
- 方案中仍有 **6 项待决策问题**（见设计方案第 18 章），其中"是否允许普通学生补充"与"卡片补充按钮如何呈现"会直接影响实现范围

## 仓库结构

```text
.
├── design/                        # 需求、设计文档与活动数据
│   ├── 产品设计思路.txt            # 需求原文
│   ├── 已知校园活动汇总.md          # 活动数据源（26 条，编号 01–26）
│   ├── 产品设计方案.md             # 产品设计方案 v3.0
│   ├── activities.seed.json       # 26 条结构化种子数据（含字段枚举定义）
│   └── README参考.md              # 本地参考文件，未纳入版本控制
├── AGENTS.md                      # 协作约定
├── .gitattributes                 # 换行符规范（仓库内统一 LF）
├── .gitignore
└── README.md
```

应用代码目录（`apps/`、`packages/` 等）尚未创建，待 P0 开发启动后补充。

## 核心设计

1. **活动卡片墙**：26 条活动预置入库并直接以卡片展示，**上线即有内容**，不依赖任何人先发布。卡片直接承载判断要素（时间、地点、门槛、截止、状态、信息完整度），26 条无需分页。
2. **组织者补充内容**：每张卡片右下角提供补充入口，组织者补齐地点、名额、费用、收获等字段并上传活动资料（PPT/Word/图片/txt/md），提交后汇入该活动的详情页。
3. **补充不覆盖、来源永远可见**：补充信息不静默覆盖平台原始数据。每个字段带来源标签（平台原始 / 组织者确认 / 学生补充·待核实），与原值不同的补充同时显示原值并进入变更记录。
4. **四层可信度分级**：按补充人身份决定直发或送审。由于 26 条中有 20 条未注明发布方，"组织者"身份需通过认领审核确认；普通学生的补充永久标注"待核实"，不并入平台字段区。
5. **"未注明" 必须显式呈现**：信息缺失显示为"未注明"，而非留空。缺失本身就是判断依据，也是给组织者的补充提示——**18 条缺失地点的活动正是补充机制的主要对象**。
6. **更新合并**：补充通知不独立成卡，作为父活动的变更记录挂载。例如数据源中的 09 是 01 的场地与时间变更，合并后只呈现一个活动。
7. **时效状态机与风险标记**：报名中 / 报名已截止（可候补）/ 已结束（含待回放）/ 长期有效 / 地点待定；过期信息默认不展示。疑似营销或诈骗的条目不予收录或强制警示，补充内容同样受此规则约束。

## 快速开始

当前无代码，建议按以下顺序阅读：

1. [design/产品设计思路.txt](design/产品设计思路.txt) —— 需求原文，先明确要解决什么问题
2. [design/产品设计方案.md](design/产品设计方案.md) —— 完整设计方案；第 3 章是对数据源的实际分析，第 4–7 章是四大核心机制
3. [design/activities.seed.json](design/activities.seed.json) —— 26 条结构化数据，`_meta` 内含字段枚举与覆盖率

**计划的技术栈**（尚未落地）：Next.js + TypeScript + Tailwind（前端）、NestJS（后端）、PostgreSQL + Prisma（数据库）、S3 兼容对象存储、Redis + BullMQ（转换队列）、LibreOffice 无头 + ClamAV（独立容器内的文档转换与扫描）、PDF.js（预览）、Docker Compose（部署）。

## 常用命令

仓库当前仅使用 Git，构建与测试命令待代码落地后补充：

```bash
git log --oneline     # 查看提交历史
git status            # 查看工作区状态
git show HEAD         # 查看最近一次改动
```

校验种子数据可解析且条目数正确（当前唯一的自动化验证）：

```powershell
(Get-Content design/activities.seed.json -Raw -Encoding UTF8 | ConvertFrom-Json).entries.Count   # 期望 26
```

## 数据说明

活动数据源为题目给定的 `已知校园活动汇总.md`（原为 .docx，已转换为 Markdown 以便版本对比与检索），其「主要内容」列是一整句散文，无独立的要求列与地点列，因此已拆解为结构化字段。

**需要预先知道的两个覆盖率事实**（26 条实测，详见设计方案 §3.2）：

- **线下明确地点仅 4 条（15%）**，另有 2 条为线上活动、2 条明确待确认、18 条未提及 → 标注机制上线后，其主要产出是**把"地点未注明"这一缺口显式暴露**，而非给出地点
- **收获（学分/工时/证书）0 条（0%）** → 决策卡的"收获"行将长期显示"未收录"，产品不编造也不留空

数据源中还包含若干真实场景的信息陷阱，方案已逐条对应处理：同一活动的重复与增量发布、已过时效的条目、信息不完整、疑似营销或诈骗、无需报名与非先到先得、非活动类的资料合集。

## 协作约定

见 [AGENTS.md](AGENTS.md)：

- 每次改动完成后，都必须创建一个对应的 Git commit，以便后续追踪和回滚
- 每次改动后，都必须编写或更新相关测试，并在交付给用户前，确保所有测试和验证全部通过

> 当前仓库尚无测试框架。上述第二条对文档类改动无执行落点；待代码落地并选定测试框架（vitest / pytest）后需一并建立。

## 更新记录

| 提交 | 日期 | 内容 |
| --- | --- | --- |
| `d872ca3` | 2026-09-19 | 设计方案升级 v3.0，改为卡片墙展示 + 组织者补充内容 |
| `f940109` | 2026-09-19 | 活动数据源由 docx 转换为 Markdown |
| `9421826` | 2026-09-19 | 将 README参考.md 标注为本地参考并加入 .gitignore |
| `b3ef2a2` | 2026-09-19 | 新增 README（设计阶段版本） |
| `6ba99c6` | 2026-09-19 | 新增 AGENTS.md，约定提交流程与测试要求 |
| `99c34f6` | 2026-09-19 | `deadline_status` 归一为枚举并登记到字段表 |
| `4389d4f` | 2026-09-19 | 设计方案升级 v2.0，新增活动库种子数据 |
| `8e8b8db` | 2026-09-19 | 新增产品设计方案 v1.0 |
| `05d9e0f` | 2026-09-19 | 初始化仓库，添加 .gitignore 与 .gitattributes |

## 安全

已完成设计、尚未实现。活动资料支持 PPT / Word / 图片 / txt / md 上传与在线预览，因此安全重点是**不可信文档的处理**：

- 扩展名白名单 + magic number 双重校验，拒绝含宏格式（`.docm` / `.pptm` / `.xlsm`）
- 单文件 ≤ 50MB、单活动 ≤ 10 个附件；ClamAV 病毒扫描，命中即隔离
- 文档转换在独立容器内执行，禁外网、限时限额
- 权限采用 RBAC + 资源级所有权校验；附件通过带签名的临时 URL 访问
- 隐私：学号仅后台可见，联系方式默认不公开，联系仅走站内私信

## 许可证

待定。仓库当前无 `LICENSE` 文件，如需开源请先补充许可证。
